...
for i in range(5):
    ...
    datamcs=np.loadtxt('wrfn1yr2.txt')[:,5112:]
...
for i in range(2):
    ...
    for j in range(6):
        ...
        datamcs=np.loadtxt('wrfn1yr2.txt')[:,5112:8784]
        if i==0:
            data[datamcs==0]=np.nan
        elif i==1:
            data[datamcs>0]=np.nan
...
fig.savefig('NormalFigureSP4new.withSta_wrfncc_mir.jpg',bbox_inches='tight',dpi=300)